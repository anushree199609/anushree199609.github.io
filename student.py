...
class Student:
    def __init__(self,root):
        ...
        title=Label(self.root,text="Add Student",bd=10,relief=GROOVE,font=("times new roman",40,"bold"),bg="lightblue",fg="black")
...
#================Manage Frame=====================================
        ...
        m_title=Label(Manage_Frame,text="Add Student",bg="crimson",fg="white",font=("times new roman",30,"bold"))
        ...
        lbl_Branch.grid(row=6, column=0, pady=10, padx=20, sticky="w")

        txt_Branch = Entry(Manage_Frame,textvariable=self.branch_var,font=("times new roman", 15, "bold"), bd=5, relief=GROOVE)
        txt_Branch.grid(row=6, column=1, pady=10, padx=20, sticky="w")
...
#=============Button Frame===========
        ...
        Addbtn=Button(btn_Frame,text="Add",width=5,command=self.add_students).grid(row=0,column=0,padx=20,pady=10)
        updatebtn=Button(btn_Frame, text="Update", width=5,command=self.update_data).grid(row=0, column=1, padx=20, pady=10)
        deletebtn=Button(btn_Frame,text="Delete",width=5,command=self.delete_data).grid(row=0,column=2,padx=20,pady=10)
        ...
        searchbtn=Button(Detail_Frame,text="Search",width=10,padx=5,command=self.search_data).grid(row=0,column=3,padx=10,pady=10)
        showallhbtn=Button(Detail_Frame,text="Show All",width=10,pady=5,command=self.fetch_data).grid(row=0,column=4,padx=10,pady=10)
...
#===============Table Frame===============================
        ...
        self.Student_table=ttk.Treeview(Table_Frame,columns=("roll","name","email","gender","contact","branch","Address"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
    ...
    def add_students(self):
         ...
         cur=con.cursor()
         cur.execute("insert into students values(%s,%s,%s,%s,%s,%s,%s)",(self.Roll_N0_var.get(),
                                                                          self.name_var.get(),
                                                                          self.email_var.get(),
                                                                          self.gender_var.get(),
    ...
    def update_data(self):
        con = pymysql.connect(host="localhost",user="root",password="",database="stm")
        cur = con.cursor()
        cur.execute("UPDATE students SET name=%s,email=%s,gender=%s,contact=%s,branch=%s,address=%s,where roll_no=%s",(
                                                                                                   self.name_var.get(),
                                                                                                   self.email_var.get(),
                                                                                                   self.gender_var.get(),
                                                                                                   self.contact_var.get(),
